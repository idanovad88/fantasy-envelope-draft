...
`data/rookies-2026.json` supplies the names, and `withRookies(players, rookieData, season)` in `lib/espnPool.mjs` appends them. Both writers go through it — `scripts/fetch-espn-ranks.mjs` and `create-league`, on the live path and the bundled-fallback path alike — so the CSV, `data/nba-pool.json` and a newly seeded league all carry the same 447 players.
...
**`POST /api/admin/add-missing-players`** — `{ league_id, players, dry_run }`. Inserts only the names the league does not have and **writes to no existing row at all**, so it is safe mid-draft and safe to point at the whole 447-row pool CSV. Idempotent: run it twice and the second run adds nothing.
