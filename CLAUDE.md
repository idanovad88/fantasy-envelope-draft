...
⚠️ **The live 2026-27 open league ("לייב אוקשן רק דראפט") is deliberately left unranked** — the league owner decided on 2026-09-03 not to re-rank a draft already in progress. Every one of its 358 players keeps `ranking = NULL`, and the pool sorts alphabetically by the secondary `name` key. `nba_players_2026_27.csv` and the update route are for **leagues created from here on**, applied at import time. Do not run the update against that league, `dry_run` or otherwise, without the owner asking for it.
