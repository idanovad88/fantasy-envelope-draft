...
⚠️ **It measures a delta against an anchor element, it does not restore a saved `scrollY`** — that is what makes it safe to ship to every browser instead of sniffing for Safari. Where the browser already anchors, the correction has landed before this looks, the delta is 0, and it does nothing. Verified both ways in a harness with `overflow-anchor` forced off and on: content removed *and* inserted above the viewport held the reader at the same pixel in both modes, with no overshoot when the browser had already done the work.

Two properties worth keeping if it is ever touched: any scroll the *reader* starts (wheel, touch, key, or iOS momentum still running after touchend — visible only as a `scroll` event) cancels the watch outright, because their position outranks anything we were about to restore; and a second refresh supersedes the first, since two watches pulling towards two different anchors would fight.
...
Only `postgres` and `service_role` may appear for a mutating one. The read-only helpers (`open_is_running`, `open_accepts_actions`, `open_clock_now`, `open_team_max_bid`, `open_team_open_slots`, `open_within_hours`, `open_last_hours_boundary`) stay public — they read nothing that is not already public through the SELECT policies. `pg_cron` is unaffected: the job runs as the function owner.
...
**The night stops the clock, not the managers.** Nominating, bidding and PASS are all accepted at any hour — the gate on all three is `open_accepts_actions()` (league `status = 'active'`), so only an admin PAUSE closes the board for input. `draft_start_hour`–`draft_end_hour` is purely a clock rule. An auction therefore *can* close overnight, when the last non-leader passes and `open_settle_auction()` finishes it — nobody was cut off by a timer there, every team still in it chose to leave. What cannot happen at night is a `timeout` close: the tick returns before its close loop whenever the league is frozen.

`open_is_running()` is now called by nothing. It is kept because older function bodies in `supabase/` still reference it, and dropping it would make re-running one of those files fail rather than merely revert behaviour.

⚠️ **Every deadline written during a freeze must be in frozen time, never `NOW()`** — that is what `open_clock_now()` is for, and there are exactly two callers: the soft-close ladder in `open_place_bid()` and the opening window in `open_nominate()`. The wall clock is wrong in both, and differently. For a bid, `deadline_at - NOW()` shrinks through the night, so every night bid would take the short window (and a fresh auction would look nearly expired), and the morning thaw would then add the whole night *on top of* the window just granted. For a nomination, a wall-clock `NOW() + open_pass_timeout_minutes` names an instant the stopped clock never reaches, and the thaw hands that auction far more than its window — scaled by how deep into the night it went up.

`open_clock_now()` returns `open_frozen_since` when it is stamped, and otherwise — if the league is active but outside its hours — `open_last_hours_boundary()`, the instant the tick is about to stamp anyway. Reading the stamp first is deliberate: a stamp still set while the league is inside its hours means the thaw has not run yet (the tick is once a minute), and the deadlines it is about to shift are still the pre-freeze ones, so the stamp is the right base then too. `frozen_since + W` becomes exactly `morning + W` once the shift lands.

**The timer cannot be sniped — a graduated soft close.** On a bid the deadline moves to `open_clock_now() + W`, where `W` is the **smallest configured window larger than the time remaining**; if neither qualifies the deadline does not move. With the defaults (`open_extend_short_minutes` 30, `open_extend_long_minutes` 60):
...
**An admin can nominate, bid and PASS in a team's name — all three, and none of them leaves a mark.** `resolveOpenActor()` takes an optional `requestedTeamId` and refuses it for anyone who is not the league admin or creator; the routes then call the same RPC they would for the team itself, so `open_place_bid()` applies the identical price floor, budget ceiling, pass check and soft-close ladder. Nothing in `open_bids` records who typed it, deliberately — the ask was for the bid to look like the team's own, and a column saying otherwise would show up in the ledger and the history for the rest of the draft.

⚠️ **`/api/open/bid` used to refuse this on purpose** ("bidding for someone else is putting words in their mouth", unlike PASS, which an admin needs to unblock a stalled auction). The league owner asked for it on 2026-09-13 for the manager who cannot reach the board during a live auction. The reasoning that blocked it has not stopped being true — it is now a policy the admin carries rather than one the code enforces.

**UI:** a team dropdown, an amount and "הצע" on every open auction in the admin board tab, directly above the PASS row that was already there. Both lists are the same set — approved, not the current leader, has not passed — because `open_place_bid()` and `open_pass()` draw exactly those two lines. The amount defaults to `current_price + 1` and the ceiling shown beside it is `getOpenMaxBid()` with the team's leading commitments deducted; display only, as everywhere else.
...
**A team can nominate in advance — the automatic queue.** A board slot frees the moment an auction closes, and until now it sat empty until a human put a player up: `open_close_auction()` settles its siblings and nothing else, so a turn arriving at 03:00 simply waited and the pace of the draft was set by who was awake. Each team now keeps an ordered list (`open_nomination_queue`) of players it wants up, each with its own opening bid, and when the turn comes the first *viable* entry goes up on its own. Having no list is how a team opts out — the turn then waits for it exactly as before.

**The list is private, and that is the security property.** Unlike every other `open_*` table (`FOR SELECT USING (true)`), this one has an own-team SELECT policy — owner or assistant, via `teams.user_id` / `teams.assistant_user_id` — and no write policy at all. A rival who could read your queue would know precisely whom to put up ahead of you. `open_queue_next_entry()` is therefore **SECURITY INVOKER**, not definer: called directly by a user it returns nothing for a team that is not theirs, while inside the definer functions it runs as their owner and sees everything.

**The fill runs at the end of `open_draft_tick()`, never from `open_close_auction()`.** `open_nominate()` ends by calling `open_settle_auction()`, which can close the auction inside the nomination itself → `open_close_auction()` → the sibling sweep → more settling; filling from there re-enters `open_nominate()` from inside that recursion. The tick already runs every minute *and* on every relevant page load through `settleOpenDraft()`, so it is immediate for anyone watching. `open_autofill_board()` is bounded by a **hard iteration cap** (`open_board_size + 2`) rather than `WHILE free > 0`, because one nomination can free the very slot it took.

**Night fills too**, and that needed a restructure. Nominating by hand runs straight through the night (`open_accepts_actions()`); only the clocks stop. The tick used to `RETURN` early while frozen, so a fill at the end of the body would silently not have run at night. The freeze/thaw branch is now `IF/ELSE` rather than an early return, and the fill is called after it whenever the league is `active` — an admin PAUSE fills nothing, since `open_nominate()` refuses it anyway.

**It takes the first *viable* entry, not the first entry.** Viable = the player is still `available` **and** the opening bid is within `open_team_max_bid()` right now. Otherwise one unaffordable name at the head of the list would block everything behind it for good. Nothing is ever deleted for being skipped: a player somebody else put up drops out of the list on sight and returns if an admin cancels that auction, and an entry priced above today's ceiling comes back the moment the team is outbid elsewhere. `NominationQueue` marks that case in orange rather than treating it as an error.

**`open_notify_turn_candidates()` excludes a team whose queue can answer.** Both jobs run every minute, so without that clause a team with a filled list gets "🏀 תורך להעלות שחקן" seconds before the board answers for it.

**UI:** an arrow (`QueueArrow`, beside `WatchStar`) on each row of the players page, plus a collapsed `NominationQueue` card above the table for order, opening bids and removal — ↑/↓ inputs, not drag-and-drop, since the repo has no dnd library and the established pattern is the admin lottery's numeric rank editor. `PlayerPicker` turns the column on by the presence of `queuedIds`, the same convention as `watchedIds`. Writes go through `POST /api/open/queue` (toggle) and `POST /api/open/queue/reorder` (whole list), both resolving the caller with `resolveOpenActor()` so an assistant counts as the team.
...
12. `supabase/migration_open_nomination_queue.sql` — the automatic nomination queue above: `open_nomination_queue`, `open_board_free_slots()`, `open_queue_next_entry()`, `open_autofill_board()`. ⚠️ It **replaces the bodies of `open_draft_tick()` and `open_notify_turn_candidates()`**, so re-running #1 or #11 after it silently reverts both — the board stops filling itself and the "your turn" push comes back for teams whose queue is about to answer. Needs no new cron; the every-minute tick already runs it.
13. `supabase/migration_open_settle_on_close.sql` — a closing auction re-settles the rest of the board (see **Auto-PASS is for teams that are out for good** above): winning changes `is_complete` and the budget, which are the two inputs to the auto-PASS test in *every other* open auction, and nothing re-ran it there. `open_close_auction()` now sweeps its siblings and `open_draft_tick()` sweeps the whole board once a minute, both `FOR UPDATE SKIP LOCKED` so a row another transaction is mid-bid on is left to it. Also folded into #1. Applied 2026-09-09 — `open_close_auction()` only: #12 had already put the sweep into the tick, so the every-minute net was live and what was missing was the immediate one at close, worth up to a minute of lag. ⚠️ Its `open_draft_tick()` is **#12's body** with the sweep folded in, `open_autofill_board()` call intact — #12 is the newer file and is already applied, so a pre-queue tick here would stop the board filling itself.
...
#### ESPN's ranked list has no rookies in it, and that is not a bug you can fix in the fetch

`fetchRankedPlayers()` skips a player with no `draftRanksByRankType.STANDARD.rank` — having a rank is what being in this list *means*. In the preseason ESPN has not ranked the class that was just drafted, so the 2026-27 pool held the entire 2025 class (Flagg at 41, Bailey 134, Harper 143, Knueppel 152, Edgecombe 170) and **not one player from the 2026 draft**. Nothing about the request was wrong; those players simply have no rank to sort by yet.

`data/rookies-2026.json` supplies the names, and `withRookies(players, rookieData, season)` in `lib/espnPool.mjs` appends them. Both writers go through it — `scripts/fetch-espn-ranks.mjs` and `create-league`, on the live path and the bundled-fallback path alike — so the CSV, `data/nba-pool.json` and a newly seeded league all carry the same 474 players.

Four properties worth keeping:

- **It is a supplement, never an override.** Any rookie ESPN *has* returned keeps his ESPN rank, position and auction value; only the genuinely absent names are appended. The file therefore needs no maintenance and no deletion once ESPN catches up — the merge goes quiet one name at a time, and `--check` reports nothing.
- **It is season-scoped.** `rookieData.season` (2027 = the 2026-27 season, named for the year it ends in) must equal the season being fetched, so `--season 2028` cannot inject the 2026 class into next year's pool. The script looks for `data/rookies-<season − 1>.json` and writes ESPN's list unchanged when there isn't one.
- **`MIN_RANKED` is checked on ESPN's answer, before the merge.** Otherwise 60 names from a local file would paper over a truncated fetch — the exact failure that guard exists to catch.
- **The appended block sorts last, in draft order** (388–447 today, with the rostered-unranked block below it). The No. 1 pick lands at 388 rather than somewhere in the 60s on purpose: ESPN has not valued these players, and inventing a rank for them would be a guess wearing the same clothes as data. Draft order is the only ordering here that is a fact. Re-rank from the players tab if a league wants them higher.

⚠️ **The names in that file did not come from ESPN, so treat a mismatch as the file's fault.** They were transcribed from 2026 draft coverage (all 60 picks, 23–24 June 2026) because the session that added them could not reach `lm-api-reads.fantasy.espn.com` at all — an egress policy, not an ESPN outage. Consequences: `nba_players_2026_27.csv` and `data/nba-pool.json` were regenerated **offline**, by merging the rookies into the ranked block already on disk, so they carry the ESPN ranks of the last real fetch and no newer ones. **Run `npm run ranks` from a machine that can reach ESPN** to pick up genuine rank movement; it will re-emit the same rows plus whatever moved. A rookie whose spelling differs from ESPN's will simply appear twice once ESPN ranks him — a duplicate name in the players tab is the symptom, and fixing the spelling in `data/rookies-2026.json` (or deleting the row from the league) is the fix.

⚠️ **The positions are coarse and are not ESPN `eligibleSlots`.** Draft coverage gives guard/forward/center, widened to the two slots that role usually covers (`PG/SG`, `SF/PF`, `PF/C`). They feed `assign_roster_slot()` like any other position, so a league that cares should narrow them — or just wait for ESPN to rank the player, at which point his real eligibility replaces this row entirely.

#### ESPN ranks 387 players and the NBA rosters 558, so the pool was missing real players

Same shape as the rookie gap above, and it survived that fix because it is not about rookies. `fetchRankedPlayers()` keeps a player only if `draftRanksByRankType.STANDARD.rank` is set — that is what a rank *is* here — and ESPN ranks **387** of the ~1100 names it knows. Everyone else is dropped silently. A manager noticed **Day'Ron Sharpe** was missing on 2026-09-11; checking the pool against all 30 real ESPN rosters turned up **163** rostered NBA players absent from it, including Cedric Coward, Collin Gillespie, Ajay Mitchell, Maxime Raynaud, Ryan Kalkbrenner, Jay Huff, Taylor Hendricks and Jaylon Tyson. Nothing in the app said so — an unranked player does not appear anywhere to be missing from.

`fetchRosteredExtras()` in `lib/espnPool.mjs` closes it, and `withExtras()` appends the result the same way `withRookies()` does — supplement, never override, sorted last. Both writers go through it, so the CSV, `data/nba-pool.json` and a newly seeded league stay identical. 27 players today (474 total).

- **It is a second request, not a bigger one.** `view=players_wl` returns 180KB for 700 players against 15.5MB for the ranked view, because it carries no `stats` — so in `create-league` it runs in `Promise.allSettled` *alongside* the ranked fetch and costs no wall clock. It is also the one part of seeding allowed to fail quietly: a league short of Day'Ron Sharpe is worth having, a league with no players is not.
- **Percent-owned is the filter, and it has to be.** ESPN's database is full of retired players, and they are not distinguishable by `proTeamId` — Ian Clark is listed on a pro team. `EXTRAS_MIN_OWNED = 0.25` was picked against the real rosters: of the 63 unranked names at or above it, 61 were on a 2026-27 roster and the two that were not were free agents, which `proTeamId > 0` then drops. Below ~0.1% the list turns into database residue (86% → 68% real).
- **Sorting by ownership means the ranked list is not what bounds the request**, so `fetchRosteredExtras()` throws when the tail of the response never drops below the threshold — that means `EXTRAS_LIMIT` cut the list off mid-way and an unknown number of qualifying players are missing. A silently partial supplement would seed every future league short, which is the failure this whole section is about.
- **Ordering is percent-owned desc, then name**, for the same reason the ranked block breaks ties on data: two consecutive `npm run ranks` runs must be byte-identical or the weekly `--check` diff is worthless. Verified after this change — same md5 twice, `0 moved, 0 dropped`.

⚠️ A free agent is deliberately absent even when a team's own roster page lists him (Brandon Williams, 0.29% owned, `proTeamId 0` at the time of writing). Add that kind of name by hand from the players tab; do not loosen `proTeamId > 0`, which is the only thing keeping retired players out.

The 135 rostered players below the threshold are real but are two-way and camp bodies. If a league wants them, the ranked-vs-roster diff is reproducible from `site.api.espn.com/apis/site/v2/sports/basketball/nba/teams/<id>/roster` — 30 small requests, no auth — and `add-missing-players` takes the result as a `name,pos` paste.

#### Adding players to a league that already exists

Three import routes, and the third one exists because the first two leave a gap: `/api/import-players` inserts **everything** with no dedupe (fine for an empty league, catastrophic for a running one), and `/api/admin/update-player-rankings` only ever `UPDATE`s, so a player the league never had stays missing. Topping up a live draft needed the third shape.

**`POST /api/admin/add-missing-players`** — `{ league_id, players, dry_run }`. Inserts only the names the league does not have and **writes to no existing row at all**, so it is safe mid-draft and safe to point at the whole 474-row pool CSV. Idempotent: run it twice and the second run adds nothing.

- Matching runs **over the stored rows, against the incoming keys** — the same direction as `update-player-rankings`, and it has to be. `matchPlayerName()`'s wildcard exists for the Latin-1 mojibake in the *stored* names (`Nikola Joki?`); matching the other way round would fail to recognise Jokić and Dončić and re-add them as duplicates. Verified against the real pool: 60 missing from an ESPN-only league, 0 from one that already has them, 0 duplicates from a fully corrupted name column.
- A player arriving **without** a rank is numbered from `MAX(ranking) + 1` **of that league**, not from the source file. A league's scale is its own — the live pool runs to 265, an older one to 294 — so importing 388 from a foreign file would mean nothing. He lands below every ranked player and above every unranked one.
- `nba_team` is always written NULL, for the reason in the team-column warning below.
...
⚠️ **Never re-run `/api/import-players` against a league that has already drafted.** It `insert`s with `status: 'available'` hardcoded and has no dedupe, so it creates a second, undrafted copy of every player, including the ones already sold. Adding players to a running league goes through `/api/admin/add-missing-players` (above). Re-ranking an existing league goes through **`POST /api/admin/update-player-rankings`** instead, which only ever `UPDATE`s, and only `ranking` (plus `nba_team` where still blank) — `status`, `drafted_by_team_id`, `draft_price` and `roster_slot` are never written. Send `dry_run: true` first and read the report; there is no dev database, so the first run hits the live league either way.

**The live 2026-27 open league ("לייב אוקשן רק דראפט") was ranked on 2026-09-04**, mid-draft, at the owner's request — reversing his decision of the day before to leave it alone. 265 of its 341 players carry a `ranking`; the remaining 76 are genuinely unranked by ESPN and sort last on the `name` key. The 68 already-drafted players were untouched, as the update route only ever writes `ranking`.

The symptom that prompted it is worth recognising on sight: **pressing "דירוג" in the players tab appeared to do nothing.** The button was fine. `usePlayerFilter` falls through to a name comparison when both `ranking`s are NULL, so with an entirely unranked pool "דירוג" and "שם" produce the identical list, and the `#` column reads "—" on every row. Diagnose that report by counting ranked rows before reading any sort code:
...
- `delete-player` — one player (`player_id`), several at once (`player_ids`), or the whole available pool (`all_available`). The league and `status = 'available'` guards live in the DELETE statement itself rather than in a verification read, so no crafted id list can reach a drafted player or another league's row; the reply reports what was actually removed, not what was asked for. `player_ids` is chunked at 100 because `.in()` is serialised into the query string.
