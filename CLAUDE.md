...
- A player arriving **without** a rank is numbered from `MAX(ranking) + 1` **of that league**, not from the source file. He lands below every ranked player and above every unranked one.

  ⚠️ A league's scale is whatever file it was last ranked from, and **it is not the number of ranked players** — a trap worth naming, because the two get confused. The live open league has *265 ranked rows*, but those ranks came from the 387-row ESPN list, so its `MAX(ranking)` sits near 387, not near 265. Reading the max off the league itself is exactly what makes the distinction stop mattering; hard-coding either number would not.
