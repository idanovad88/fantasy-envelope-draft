...
**The night stops the clock, not the managers.** Bids and PASS are accepted at any hour — the gate on both is `open_accepts_actions()` (league `status = 'active'`), so only an admin PAUSE closes the board for input. **Nomination is the exception**: `open_nominate()` keeps `open_is_running()`, so new players go up only during active hours. An auction therefore *can* close overnight, when the last non-leader passes and `open_settle_auction()` finishes it — nobody was cut off by a timer there, every team still in it chose to leave. What cannot happen at night is a `timeout` close: the tick returns before its close loop whenever the league is frozen.

⚠️ **A deadline written during a freeze must be in frozen time, never `NOW()`** — that is what `open_clock_now()` is for, and `open_place_bid()` is the only caller. The wall clock is wrong twice over at night: `deadline_at - NOW()` shrinks through the night, so every night bid would take the short window (and a fresh auction would look nearly expired), and the morning thaw would then add the whole night *on top of* the window just granted. `open_clock_now()` returns `open_frozen_since` when it is stamped, and otherwise — if the league is active but outside its hours — `open_last_hours_boundary()`, the instant the tick is about to stamp anyway. Reading the stamp first is deliberate: a stamp still set while the league is inside its hours means the thaw has not run yet (the tick is once a minute), and the deadlines it is about to shift are still the pre-freeze ones, so the stamp is the right base then too. `frozen_since + W` becomes exactly `morning + W` once the shift lands.
