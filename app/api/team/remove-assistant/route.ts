...
// Remove a team's assistant manager. Allowed for the team owner OR the league admin/creator.
export async function POST(req: Request) {
  ...
  const { data: team } = await admin
    .from('teams').select('id, league_id, user_id').eq('id', teamId).maybeSingle()
  if (!team) return NextResponse.json({ error: 'קבוצה לא נמצאה' }, { status: 404 })

  // Authorize: team owner, OR admin for this league, OR league creator (mirrors set-team-admin)
