...
export async function POST(req: Request) {
  ...
  if (!leagueId) return NextResponse.json({ error: 'חסרים פרמטרים' }, { status: 400 })
  ...
  await admin.from('leagues').update({ logo_url: null }).eq('id', leagueId)

  // Best-effort cleanup from storage
  await admin.storage.from('draft-media').remove([`league-logos/${leagueId}`])
