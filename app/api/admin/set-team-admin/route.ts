...
export async function POST(req: Request) {
  ...
  // Verify caller is admin or creator of the team's league
  const [{ data: callerAdmin }, { data: ownedLeague }] = await Promise.all([
    admin.from('admin_users').select('league_id').eq('user_id', user.id).eq('league_id', team.league_id).maybeSingle(),
    supabase.from('leagues').select('id').eq('id', team.league_id).eq('created_by', user.id).maybeSingle(),
  ])
